...
namespace kovra {
void Material::update_push_constants(
    vk::CommandBuffer cmd, vk::ShaderStageFlags stages,
    const std::vector<uint8_t> &data) const {
    cmd.pushConstants(
        pipeline_layout.get(), stages, 0, data.size() * sizeof(uint8_t),
        data.data());
}
void Material::bind_pipeline(vk::CommandBuffer cmd) const {
    cmd.bindPipeline(pipeline_bind_point, pipeline.get());
}
void Material::bind_desc_sets(
    ...
    cmd.bindDescriptorSets(
        pipeline_bind_point, pipeline_layout.get(), first_set, desc_sets,
        dynamic_offsets);
}

GraphicsMaterialBuilder::GraphicsMaterialBuilder()
    : vertex_input_desc{Vertex::get_vertex_desc()},
      vertex_input_ci{
...
Material GraphicsMaterialBuilder::build(const vk::Device &device) {
    ...
    return Material{
        std::move(pipeline.value), std::move(*pipeline_layout),
...
GraphicsMaterialBuilder &GraphicsMaterialBuilder::set_pipeline_layout(
...
GraphicsMaterialBuilder &GraphicsMaterialBuilder::set_cull_mode(
    vk::CullModeFlags cull_mode, vk::FrontFace front_face) {
    rasterization_ci.setCullMode(cull_mode);
    rasterization_ci.setFrontFace(front_face);
    return *this;
}
GraphicsMaterialBuilder &GraphicsMaterialBuilder::disable_multisampling() {
...
GraphicsMaterialBuilder &GraphicsMaterialBuilder::disable_blending() {
...
GraphicsMaterialBuilder &GraphicsMaterialBuilder::enable_alpha_blending() {
    color_blend_attachment.setColorWriteMask(
        vk::ColorComponentFlagBits::eR | vk::ColorComponentFlagBits::eG |
        vk::ColorComponentFlagBits::eB | vk::ColorComponentFlagBits::eA);
...
GraphicsMaterialBuilder &GraphicsMaterialBuilder::enable_additive_blending() {
    color_blend_attachment.setColorWriteMask(
        vk::ColorComponentFlagBits::eR | vk::ColorComponentFlagBits::eG |
        vk::ColorComponentFlagBits::eB | vk::ColorComponentFlagBits::eA);
    color_blend_attachment.setBlendEnable(vk::True);
...
Material ComputeMaterialBuilder::build(const vk::Device &device) {
    if (!shader.has_value() || !pipeline_layout.has_value()) {
        throw std::runtime_error(
...
ComputeMaterialBuilder &ComputeMaterialBuilder::set_pipeline_layout(
    vk::UniquePipelineLayout pipeline_layout) {
    // Destroy previous pipeline layout if it exists
    if (this->pipeline_layout.has_value()) {
        this->pipeline_layout.reset();

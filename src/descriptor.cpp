...
DescriptorSetLayoutBuilder::build_unique(const vk::Device &device) const
{
...
void
DescriptorAllocator::clear_pools(const vk::Device &device)
{
    // Insert all full pools into the ready pools list
    ready_pools.insert(
      ready_pools.end(),
      std::make_move_iterator(full_pools.begin()),
...
void
DescriptorAllocator::destroy_pools()
{
    for (auto &pool : ready_pools) {
        pool.reset();
    }
    ...
    for (const auto &ratio : ratios) {
        pool_sizes.push_back(
          vk::DescriptorPoolSize{}
            ...
            .setDescriptorCount(static_cast<uint32_t>(set_count * ratio.ratio))
        );
    }
